(function() {
  var HSEXPRESS, P, SeqStart, SeqStop, fs, path,
  ...
  module.exports = HSEXPRESS = (function(superClass) {
    ...
    HSEXPRESS.prototype.expressStatus = function(req, res) {
      var me, message;
      me = this;
      console.log('expressStatus', 'status', this.lastState);
      if (me.hasMachine) {
        res.send(JSON.stringify({
          success: false,
          msg: "Keine Verbindung zur Maschine"
    ...
    HSEXPRESS.prototype.expressStopJob = function(req, res) {
      ...
      seq = new SeqStop();
      seq.run(this.args.machine_ip, this.args.machine_port, 1);
      return res.send(JSON.stringify({
        success: true,
        msg: me.lastState
      }));
    ...
    HSEXPRESS.prototype.expressStartJob = function(req, res) {
      var adv, bodymessage, e, endorsement1, endorsement2, error, error1, k, me, message, seq, v;
      me = this;
      try {
        if (me.lastState.print_job_active === 1) {
          ...
          seq = new SeqStart();
          seq.quiet = false;
          seq.on('sequence_message', function(msg) {
            return console.log('sequence_message', msg);
          });
          seq.on('sequence_error', function(err) {
            return console.log(err);
          });
          seq.on('sequence_timeout', function(err) {
            return console.log(err);
          });
          ...
          return res.send(JSON.stringify({
            success: true,
            msg: "Job gestartet",
            ip: this.args.machine_ip,
            port: this.args.machine_port
          }));
        }
      } catch (error1) {
        e = error1;
